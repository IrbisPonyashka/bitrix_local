...
BX.ready( () => {
    BX.addCustomEvent('Grid::thereEditedRows', () => {
		document.querySelectorAll('input[name=AGREED_TIME]').forEach(input => {
		...
		Inputmask({
            ...
            alias: "datetime",
            hourFormat: "24"
        }).mask(document.querySelectorAll('input[name=AGREED_TIME]'));
    });


    BX.addCustomEvent('Grid::beforeRequest', function(gridData, response, eventObject)
    {
		if(response.data.FIELDS && Object.keys(response.data.FIELDS).length){
			...
			for (const key in reponseFields) {
				if(!Number.isInteger(Number(key)) ){
					continue;
				}
				if(Object.keys(reponseFields[key]).length < 1){
					return;
				}

				const row = rows.getById(key);
				var rowData = {};
				...
				for (var index in cells) {
					if(Number.isInteger(Number(index))){
						...
						if(!Number.isInteger(cell)){
							if(cell.querySelector("[data-value]")){
								rowData.TASK_ID = cell.querySelector("[data-value]").getAttribute("data-value");
								rowData.AGREED_TIME = reponseFields[key]["AGREED_TIME"];
								rowData.USER_ID = BX.message('USER_ID');
								// rowData.AGREED_TIME = HHMMToSeconds(reponseFields[key]["AGREED_TIME"]);
							}
						}
					}
				}
			...
			if(rowDataArray && rowDataArray.length){
				// console.log(rowDataArray);
				// return;
				BX.ajax({
					url: "/local/components/micros/task.timesheets.grid/ajax.php?action=add&type=agreed_time", 
    ...
    BX.addCustomEvent('Grid::thereSelectedRows', (grid) => {
...
async function sendToApproval(rows) {
	...
	let grid = BX.Main.gridManager.getById('timesheets_grid').instance; // Замените 'timesheets_grid' на ваш идентификатор грида
	let filterInstance = BX.Main.filterManager.getById('timesheets_grid'); // Идентификатор грида
	let selectedRows = grid.getRows().getSelected();
	if (filterInstance) {
		if( filterInstance.getFilterFieldsValues().CREATED_DATE_datesel == "NONE"){
			BX.UI.Dialogs.MessageBox.alert(
				`<div class=&quot;ui-alert ui-alert-success&quot;>
					<span class=&quot;ui-alert-message&quot;><strong>Необходимо выбрать период задач через фильтр</span>
				</div>`
			);
		}

		var createdDatePeriodValue = getCreatedDateValue(filterInstance.getFilterFieldsValues());

		var responseData = await prepareFieldsToSendReports(selectedRows);
		
		if( Array.isArray(responseData) && responseData.length) {
			const gridRealtime = BX.Grid.Realtime = grid.getRealtime();
			let bitrixAlert = new BX.UI.Dialogs.MessageBox({
				title: "Отправка задач",
			...
			BX.ajax({
				...
				onsuccess: function(data) {
					...
					if( data.success){
						...
						BX.UI.Dialogs.MessageBox.alert(data.message);
					}
					setTimeout(() => {
						grid.editSelectedSave();
					}, 600);
				...
				onfailure: function(data) { 
					BX.UI.Dialogs.MessageBox.alert("Что-то пошло не так");
					setTimeout(() => {
						grid.editSelectedSave();
					}, 600);
				}
		...
		console.error('Filter instance not found');
	}
	...
	function getDateRangeFromSelection(selection)
	{
		...
		switch (selection) {
			...
			case 'CURRENT_WEEK':
				return [
					now.clone().startOf('isoWeek').startOf('day'), // Понедельник текущей недели
					now.clone().endOf('isoWeek').endOf('day') // Воскресенье текущей недели
				];
			case 'CURRENT_MONTH':
			...
			case 'LAST_WEEK':
				return [
					now.clone().subtract(1, 'weeks').startOf('isoWeek').startOf('day'),
					now.clone().subtract(1, 'weeks').endOf('isoWeek').endOf('day')
				];
			case 'LAST_MONTH':
				return [now.clone().subtract(1, 'months').startOf('month'), now.clone().subtract(1, 'months').endOf('month')];
			case 'NEXT_WEEK':
				return [
					now.clone().add(1, 'weeks').startOf('isoWeek').startOf('day'),
					now.clone().add(1, 'weeks').endOf('isoWeek').endOf('day')
				];
			case 'NEXT_MONTH':
