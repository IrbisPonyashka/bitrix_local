...
class CrmItemListComponent extends Bitrix\Crm\Component\ItemList
{
	...
	public function executeComponent()
	{
		...
		$this->arResult['toolbar_parameters'] = $this->getToolbarParameters();
	...
	{
		if($listFilter["UF_CRM_5_PROJECT_LINK"])
		{
			
            preg_match('/\d+/', $listFilter["UF_CRM_5_PROJECT_LINK"], $matches);
            $number = isset($matches[0]) ? (int)$matches[0] : null;

			$listFilter[0] = [
				"LOGIC" => "OR",
				[
					"=UF_CRM_5_PROJECT_LINK" => $number
				]
			];
			unset($listFilter["UF_CRM_5_PROJECT_LINK"]);
			// unset($listFilter[0]["LOGIC"]);
		}
	...
	protected function getListFilter(): array
	{
		...
		// 	$filter["UF_CRM_5_PROJECT_LINK"] = $_REQUEST["PROJECT"];
		// }
	...
	protected function processExport(): array
	{
		...
		for ($i=0; $i < count($grid["ROWS"]); $i++)
		{
			if($grid["ROWS"][$i]["data"]["UF_CRM_5_PROJECT_LINK"])
			{
				$project = $this->getProject ( $grid["ROWS"][$i]["data"]["UF_CRM_5_PROJECT_LINK"] ) ;
				$grid["ROWS"][$i]["columns"]["UF_CRM_5_PROJECT_LINK"] = $project ? $project["NAME"] : "" ; 
			}
		}
