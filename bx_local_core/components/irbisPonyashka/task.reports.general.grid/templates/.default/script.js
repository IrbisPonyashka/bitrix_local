...
BX.ready( () => {
	// let filter = BX.Main.filterManager.getById('reports_general_grid');
	// let values = filter.getFilterFieldsValues();

	// values['FILTER_GROUP_ID'] = "SG9";
	// // values['FILTER_GROUP_ID_LABEL'] = "";

	// filter.getApi().setFields(values);
	// filter.getApi().apply();
	// var generealReportsGrid = BX.Main.gridManager.getById('reports_general_grid')

    // generealReportsGrid.on('save', function(event) {
	// 	console.log(event);
    // });



    BX.addCustomEvent('BX.Main.Filter:apply', (grid_id, action, filterObj) => {
		var generealReportsGrid = BX.Main.gridManager.getById('reports_general_grid')?.instance
		if(grid_id == "reports_general_grid")
		{
			var extensesGrid = BX.Main.gridManager.getById('crm-type-item-list-190-8')?.instance
			
			var filterValues = filterObj.getFilterFieldsValues();
			var secondGridFilterValues = {};
			for (var key in filterValues) {
				if (filterValues.hasOwnProperty(key)) {
					secondGridFilterValues['FILTER_' + key.toUpperCase()] = filterValues[key];
				}
			}
			console.log(secondGridFilterValues);
            if (extensesGrid) {
                // Применяем фильтр ко второму гриду
                extensesGrid.reloadTable('POST', {
                    apply_filter: 'Y',
                    clear_nav: 'Y',
                    grid_filter_id: "crm-type-item-list-190-8",
                    grid_filter: filterValues
                });
            }
		}
	});
    ...
    // Слушаем событие при редактировании строки
    BX.addCustomEvent( 'Grid::beforeRequest', (gridData, response) => {
			...
			for (const key in reponseFields) {
					...
					if(Number.isInteger(Number(index)))
					{
						...
						if(!Number.isInteger(cell))
						{
							...
							if(cell.querySelector("[data-value]")){
								rowData.INVOICE_STATUS = reponseFields[key]["INVOICE_STATUS"];
							}else if(cell.querySelector("[task-id]")){
								rowData.TASK_ID = cell.querySelector("[task-id]").getAttribute("task-id");
							}
						}
					}
				}
				...
				BX.ajax.runAction("tasks.task.update", {
					data: {
						...
						fields: {
							"TITLE" : "test",
							"UF_INVOICE_STATUS": rowData.INVOICE_STATUS,
						}
					}
				}).then( async (response) => { 
