...
BX.ready( () => {
    BX.addCustomEvent('Grid::thereEditedRows', () => {
		document.querySelectorAll('input[name=MAIN_AGREED_TIME]').forEach(input => {
			input.value = secondsToHHMM(input.value);
		});
        Inputmask({
            mask: "99:99",
            placeholder: "HH:MM",
            alias: "datetime",
            hourFormat: "24"
        }).mask(document.querySelectorAll('input[name=MAIN_AGREED_TIME]'));
    });


    BX.addCustomEvent('Grid::beforeRequest', async (gridData, response) => 
    {
		if(response.data.FIELDS && Object.keys(response.data.FIELDS).length){
			var reponseFields = response.data.FIELDS;
			if(reponseFields){
				let rowsToUpdate = await sendUpdateReportsAgridTime(reponseFields);
				if(rowsToUpdate){
					const grid = BX.Main.grid = BX.Main.gridManager.getById('reports_detail_grid')?.instance;
					
					console.log(gridData,reponseFields);
					// document.querySelector('#AGREED_TIME_SUM_TEXT')
					// document.querySelector('#EMPLOYEY_TIME_SUM_TEXT')
					// if( rowsToUpdate.success){
					// 	BX.UI.Dialogs.MessageBox.alert("Данные успешно сохранены");
					// }else{
					// 	BX.UI.Dialogs.MessageBox.alert("data.error_message");
					// }
					setTimeout(() => {
...
async function updateSelected()
{
	let msgeBoxObj = {
		...
		message:"Это займет некоторое время",
		modal: true, 
		mediumButtonSize:false
	};

	let bitrixAlert = new BX.UI.Dialogs.MessageBox(msgeBoxObj);
	bitrixAlert.show();

	let grid = BX.Main.gridManager.getById('reports_detail_grid').instance; // Замените 'timesheets_grid' на ваш идентификатор грида
	let allRows = grid.getRows();
	let selectedRows = grid.getRows().getSelected();
	let unSelectedRows = removeRowsByIds(allRows.rows, grid.getRows().getSelectedIds() );
	
	if(selectedRows && unSelectedRows)
	{
		let mergedRows = [...unSelectedRows,  ...selectedRows];
		rowsToUpdateResult = await sendRowsToChangeStatus(mergedRows);
		
		if(rowsToUpdateResult)
		{
			const grid = BX.Main.grid = BX.Main.gridManager.getById('reports_detail_grid')?.instance;
			bitrixAlert.close();

			if( rowsToUpdateResult.success) {
				BX.UI.Dialogs.MessageBox.alert("Выбранные задачи успешно отправлены");
			}else{
				BX.UI.Dialogs.MessageBox.alert(JSON.parse(rowsToUpdateResult.error));
			}
			setTimeout(() => {
				grid.editSelectedSave();
			}, 600);
...
async function removeSelected()
{
	...
	let selectedRows = grid.getRows().getSelected();
	if(selectedRows){
		rowsToRemoveResult = await sendRowsToRemove(selectedRows);
		console.log(rowsToRemoveResult);
		if( rowsToRemoveResult.success == true){
			BX.UI.Dialogs.MessageBox.alert("Выбранные записи удалены");
		}else{
			BX.UI.Dialogs.MessageBox.alert("data.error_message");
		}
		setTimeout(() => {
...
async function sendUpdateReportsAgridTime(selectedRows)
{
	let result = {};
	for (const key in selectedRows) {

		if(Object.keys(selectedRows[key]).length < 1){
			return;
		}

		if (Object.hasOwnProperty.call(selectedRows, key)) {
			const element = selectedRows[key];
			let data = {
				id:key,
				data:{
					"AGREED_TIME":HHMMToSeconds(element.MAIN_AGREED_TIME)
				}
			};
			let updateResponse = await updateReportsById(data);
			if(updateResponse){
				updateResponse == "true" ? result.success = true : (result.success = false, result.error = updateResponse);
			}
...
async function sendRowsToChangeStatus(selectedRows)
{
	let result = {};
	for (const key in selectedRows)
	{
		if (Object.hasOwnProperty.call(selectedRows, key))
		{
			const element = selectedRows[key];
			let data = {
				id:selectedRows[key].getId(),
				data:{
					"STATUS": element.isSelected() ? 1 : 2 
				}
			};
...
async function sendRowsToRemove(selectedRows)
{
	let result = {};
	for (const key in selectedRows) {
		if (Object.hasOwnProperty.call(selectedRows, key)) {
			let deleteResponse = await deleteReportsById({id:selectedRows[key].getId()});
			console.log(deleteResponse);
			if(deleteResponse){
				deleteResponse == "true" ? result.success = true : (result.success = false, result.error = deleteResponse);
			}
...
async function deleteReportsById(rowData)
{
	return new Promise((resolve, reject) => {
		BX.ajax({
			url: '/local/components/micros/task.timesheets.grid/ajax.php?action=delete&type=reports',
...
async function updateReportsById(rowData)
{
	return new Promise((resolve, reject) => {
		BX.ajax({
			url: '/local/components/micros/task.timesheets.grid/ajax.php?action=update&type=reports',
			data : JSON.stringify(rowData),
			method: "POST",
			dataType: "json", 
			processData: false,
...
function removeRowsByIds(rows, ids) {
	if( rows.length && ids.length){
        return rows.filter( (row) => Number.isInteger(Number(row.getId())) && !ids.includes(row.getId()) );
	}
}

function secondsToHHMM(seconds) {
    // Получаем количество часов и оставшихся секунд
    const hours = Math.floor(seconds / 3600);
    const remainingSeconds = seconds % 3600;
    
    // Получаем количество минут и оставшихся секунд
    const minutes = Math.floor(remainingSeconds / 60);
    
    // Форматируем часы и минуты, добавляя ведущий ноль, если необходимо
...
function sumTimes(time1, time2) {
    ...
    // Функция для преобразования минут обратно в формат HH:MM
    ...
    // Преобразуем оба времени в минуты
    const totalMinutes = timeToMinutes(time1) + timeToMinutes(time2);

    // Преобразуем общий результат обратно в формат HH:MM
